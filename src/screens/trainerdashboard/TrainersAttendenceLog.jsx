// src/screens/trainer/TrainerAttendanceLog.js
...
/* ═══════════════════════════════════════════════════════════════ */
const WEEK_DAYS = [
  { day: 'Mon', date: '13 Jan', status: 'present',  checkIn: '6:00 AM', checkOut: '2:00 PM', duration: '8h 00m', membersHandled: 12 },
  { day: 'Tue', date: '14 Jan', status: 'present',  checkIn: '5:45 AM', checkOut: '1:30 PM', duration: '7h 45m', membersHandled: 15 },
  { day: 'Wed', date: '15 Jan', status: 'present',  checkIn: '6:15 AM', checkOut: '2:15 PM', duration: '8h 00m', membersHandled: 11 },
  { day: 'Thu', date: '16 Jan', status: 'absent',   checkIn: null,       checkOut: null,       duration: null,     membersHandled: 0  },
  { day: 'Fri', date: '17 Jan', status: 'present',  checkIn: '5:30 AM', checkOut: '1:45 PM', duration: '8h 15m', membersHandled: 14 },
  { day: 'Sat', date: '18 Jan', status: 'present',  checkIn: '6:00 AM', checkOut: '12:00 PM',duration: '6h 00m', membersHandled: 18 },
  { day: 'Sun', date: '19 Jan', status: 'off',      checkIn: null,       checkOut: null,       duration: null,     membersHandled: 0  },
];

const MONTH_WEEKS = [
  { week: 'Week 1', present: 5, absent: 1, off: 1, avgDuration: '7h 45m', totalMembers: 68  },
  { week: 'Week 2', present: 6, absent: 0, off: 1, avgDuration: '8h 10m', totalMembers: 82  },
  { week: 'Week 3', present: 5, absent: 1, off: 1, avgDuration: '7h 30m', totalMembers: 71  },
  { week: 'Week 4', present: 6, absent: 0, off: 1, avgDuration: '8h 20m', totalMembers: 90  },
];

const YEAR_MONTHS = [
  { month: 'Jan',  present: 24, absent: 2, total: 26, pct: 92 },
  { month: 'Feb',  present: 22, absent: 2, total: 24, pct: 92 },
  { month: 'Mar',  present: 25, absent: 2, total: 27, pct: 93 },
  { month: 'Apr',  present: 23, absent: 3, total: 26, pct: 88 },
  { month: 'May',  present: 26, absent: 0, total: 26, pct: 100},
  { month: 'Jun',  present: 24, absent: 2, total: 26, pct: 92 },
  { month: 'Jul',  present: 25, absent: 2, total: 27, pct: 93 },
  { month: 'Aug',  present: 26, absent: 1, total: 27, pct: 96 },
  { month: 'Sep',  present: 23, absent: 3, total: 26, pct: 88 },
  { month: 'Oct',  present: 27, absent: 0, total: 27, pct: 100},
  { month: 'Nov',  present: 24, absent: 2, total: 26, pct: 92 },
  { month: 'Dec',  present: 21, absent: 2, total: 23, pct: 91 },
];

const SUMMARY = {
  week:  { present: 5, absent: 1, off: 1, totalDuration: '38h 00m', avgDuration: '7h 36m', streak: 3,  bestStreak: 12, totalMembers: 70  },
  month: { present: 22, absent: 2, off: 4, totalDuration: '168h 30m',avgDuration: '7h 40m', streak: 12, bestStreak: 18, totalMembers: 311 },
  year:  { present: 290, absent: 19, off: 52, totalDuration: '2200h', avgDuration: '7h 35m', streak: 12, bestStreak: 28, totalMembers: 3800},
};

/* ═══════════════════════════════════════════════════════════════ */
/* CUSTOM HEADER (same as TrainerDashboard)                       */
/* ═══════════════════════════════════════════════════════════════ */
...
const DayRow = ({ item, isLast }) => {
  ...
  const label     = isPresent ? 'Present' : isAbsent ? 'Absent' : 'Day Off';
      ...
      <View style={{ flex: 1 }}>
        ...
        {isPresent && (
          <View style={st.dayTimesRow}>
            ...
            {item.membersHandled > 0 && (
              <View style={st.dayTimeChip}>
                <HugeiconsIcon icon={UserGroupIcon} size={ms(9)} color={CYAN} />
                <Text style={st.dayTimeText}>{item.membersHandled}</Text>
              </View>
            )}
          </View>
        )}
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={st.weekStatsRow}>
          ...
          <View style={st.weekStatChip}>
            ...
            <Text style={st.weekStatText}>{item.off}Off</Text>
          </View>
          <View style={st.weekStatChip}>
            <HugeiconsIcon icon={UserGroupIcon} size={ms(9)} color={CYAN} />
            <Text style={st.weekStatText}>{item.totalMembers}</Text>
          </View>
        </View>
      </View>
...
const MonthRow = ({ item, isLast }) => {
  const barColor =
    item.pct >= 90 ? GREEN : item.pct >= 75 ? GOLD : RED;
...
/* MAIN SCREEN                                                     */
/* ═══════════════════════════════════════════════════════════════ */
const TrainerAttendanceLog = ({ navigation, route }) => {
    ...
    <ImageBackground
      ...
      <LinearGradient
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['top']}>
          ...
          {/* HEADER */}
          <DashboardHeader navigation={navigation} userName={trainer.name} />
          ...
          <ScrollView
            ...
            <View style={st.pageTitleRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={st.pageLabel}>TRAINER ATTENDANCE</Text>
              ...
              <View style={st.trainerPill}>
                <HugeiconsIcon
                  ...
                  color={TRAINER_COLOR}
                />
                <Text style={st.trainerPillText}>TRAINER</Text>
              </View>
            </View>
            ...
            <View style={st.summaryCard}>
              ...
              <View style={st.summaryBottom}>
                ...
                <View style={st.summaryBottomDivider} />
                <View style={st.summaryBottomItem}>
                  <Text style={st.summaryBottomLabel}>Members</Text>
                  <Text style={[st.summaryBottomValue, { color: CYAN }]}>
            ...
            {pct >= 90 && (
              <LinearGradient
                ...
                <View style={{ flex: 1, gap: vs(3) }}>
                  <Text style={st.perfTitle}>Excellent Attendance! 🏆</Text>
                  <Text style={st.perfSub}>
                    Your attendance rate is above 90%. Keep it up!
                  </Text>
                </View>
              </LinearGradient>
            )}
            ...
            {pct < 90 && pct >= 75 && (
              <LinearGradient
                ...
                <View style={{ flex: 1, gap: vs(3) }}>
                  ...
                  <Text style={st.perfSub}>
                    {100 - pct}% away from excellent. Stay consistent!
                  </Text>
                </View>
              </LinearGradient>
            )}
            ...
            {pct < 75 && (
              <LinearGradient
                ...
                <View style={{ flex: 1, gap: vs(3) }}>
                  ...
                  <Text style={st.perfSub}>
                    Your attendance is below 75%. Try to be more consistent.
                  </Text>
                </View>
              </LinearGradient>
            )}
            ...
            <View style={st.legendCard}>
              ...
              <View style={st.legendRow}>
                {[
                  ...
                  { color: Colors.zinc[600], label: 'Day Off' },
                ].map((item, idx) => (
          ...
          <BottomNav
            ...
            onTabChange={tab => {
              if (tab === 'home')       navigation.navigate('TrainerDashboard');
              if (tab === 'attendance') {}
